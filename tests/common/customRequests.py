...
class Requests(HttpLocust):
	...
	def Post(self, url, data={}, sessionId = None, ifMatch = None):
		if (sessionId is None and ifMatch is None):
			...
			return self.client.post(url, json=data, headers={'Content-type': 'application/json', 'Authorization': str(sessionId), 'If-Match': str(ifMatch)} , catch_response=True)
	...
	def Put(self, url, data={}, sessionId = None, ifMatch = None):
		if (sessionId is None and ifMatch is None):
			...
			return self.client.put(url, headers={'Content-type': 'application/json', 'Authorization': str(sessionId), 'If-Match': str(ifMatch)}, catch_response=True)	
	...
	def Patch(self, url, data={}, sessionId = None, ifMatch = None):
		if (sessionId is None and ifMatch is None):
			...
			return self.client.patch(url, json=data, headers={'Content-type': 'application/json', 'Authorization': str(sessionId), 'If-Match': str(ifMatch)}, catch_response=True)
